...
import { Message, SystemMessage } from './types.js';
...
export class ConversationCompactor {
  ...
  /**
   * Generate compact prompt for summarization
   */
  private getCompactPrompt(customInstructions?: string): string {
...
${customInstructions}`;
   ...
   */
  private extractSummary(content: Anthropic.ContentBlock[]): string | null {
  ...
  /**
   * Compact conversation by generating summary
   */
  async compact(
    messages: Message[],
    ...
    const response = await this.client.messages.create({
      model: this.model,
      max_tokens: 8000,  // Allow detailed summary
      messages: [
        ...messages,
        {
          role: 'user',
...
Please continue the conversation from where we left it off without asking the user any further questions. Continue with the last task that you were asked to work on.`,
    ...
    const boundaryMarker: SystemMessage = {
      type: 'system',
  ...
  /**
   * Check if messages contain a boundary marker and slice from there
   */
  findLastBoundaryIndex(messages: any[]): number {
